 ...
 */
@Injectable()
export class NetworkAccessHelper {
  constructor(private readonly logger: PinoLogger) {
    this.logger.setContext(NetworkAccessHelper.name);
  }
   ...
   * @returns 是否匹配指定 IP
   */
  isServerIp(targetIp: string): boolean {
    try {
      ...
      // this.logger.error(`检查服务器 IP 失败: ${error instanceof Error ? error.message : 'Unknown error'}`);
      return false;
    }
  }
