// 文件位置：src/usecases/course/workflows/list-learner-enrolled-session-ids-by-series.usecase.ts
...
export interface ListLearnerEnrolledSessionIdsBySeriesInput {
  readonly session: UsecaseSession;
  readonly seriesId: number;
  readonly learnerId: number;
}

export interface ListLearnerEnrolledSessionIdsBySeriesOutput {
 ...
 */
@Injectable()
export class ListLearnerEnrolledSessionIdsBySeriesUsecase {
  constructor(
   ...
   */
  async execute(
    input: ListLearnerEnrolledSessionIdsBySeriesInput,
  ): Promise<ListLearnerEnrolledSessionIdsBySeriesOutput> {
    const learner = await this.requireLearner(input.learnerId);
    await this.assertAccess(input.session, learner.customerId);
    const sessionIds = await this.enrollmentService.listActiveSessionIdsByLearnerAndSeries({
