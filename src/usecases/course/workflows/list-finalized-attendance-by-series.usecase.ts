// 文件位置：src/usecases/course/workflows/list-finalized-attendance-by-series.usecase.ts
...
export interface ListFinalizedAttendanceBySeriesInput {
  readonly session: UsecaseSession;
  readonly seriesId: number;
}

export interface ListFinalizedAttendanceBySeriesOutput {
  readonly items: ReadonlyArray<FinalizedAttendanceRecord>;
}

export type FinalizedAttendanceRecord = {
...
@Injectable()
export class ListFinalizedAttendanceBySeriesUsecase {
  constructor(
  ...
  /**
   * 执行按 seriesId 查询已终审 attendance 列表
   * @param input 输入参数
   */
  async execute(
    input: ListFinalizedAttendanceBySeriesInput,
  ): Promise<ListFinalizedAttendanceBySeriesOutput> {
    ...
    const records = await this.attendanceService.listFinalizedRecordsBySessionIds({
      sessionIds,
    });
    if (records.length === 0) {
      return { items: [] };
  ...
  private ensurePermissions(session: UsecaseSession): void {
    ...
    if (!ok) {
      throw new DomainError(PERMISSION_ERROR.ACCESS_DENIED, '无权查看已终审出勤记录列表');
    }
  }
}
