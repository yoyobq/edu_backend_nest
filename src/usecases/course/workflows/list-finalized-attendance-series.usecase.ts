// 文件位置：src/usecases/course/workflows/list-finalized-attendance-series.usecase.ts
...
import {
  ...
  type FinalizedAttendanceSeriesSummary,
} from '@src/modules/participation/attendance/participation-attendance.service';
import type { UsecaseSession } from '@src/types/auth/session.types';

export interface ListFinalizedAttendanceSeriesInput {
  readonly session: UsecaseSession;
}

export interface ListFinalizedAttendanceSeriesOutput {
  readonly items: ReadonlyArray<FinalizedAttendanceSeriesItem>;
}

export type FinalizedAttendanceSeriesItem = FinalizedAttendanceSeriesSummary & {
  readonly leadCoachName: string | null;
};

@Injectable()
export class ListFinalizedAttendanceSeriesUsecase {
  constructor(
   ...
   */
  async execute(
    input: ListFinalizedAttendanceSeriesInput,
  ): Promise<ListFinalizedAttendanceSeriesOutput> {
    this.ensurePermissions(input.session);
    const summaries = await this.attendanceService.listFinalizedSeriesSummaries();
  ...
  private ensurePermissions(session: UsecaseSession): void {
    ...
    if (!ok) {
      throw new DomainError(PERMISSION_ERROR.ACCESS_DENIED, '无权查看已终审出勤的开课班列表');
    }
  }
}
